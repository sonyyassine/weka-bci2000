...
%   $Revision: 1111 $  $Date: 2006-05-03 06:23:39 +1000 (Wed, 03 May 2006) $
...
if nargout > 1
    ...
    if corrXX
         ...
         % the exact dist'n.  Kendall's tau also may not be exactly one
         % along the diagonal when there are ties.
...
for i = 1:p1
    ...
    for j = j0:p2
        ...
        case 'p' % Pearson's linear correlation
            x0 = xi - mean(xi);
            y0 = yj - mean(yj);
            coef(i,j) = (x0./norm(x0))' * (y0 ./ norm(y0));

                                   ...
                                   SignalProperties& outSignalProperties ) const
{
  ...
  // Input signal checks.
  for( int i = 0; i < Parameter( "MUD" )->NumRows(); ++i )
  {
  ...
  for( int i = 0; i < Parameter( "MLR" )->NumRows(); ++i )
  {
...
// Returns:    N/A
// **************************************************************************
void ClassFilter::Initialize( const SignalProperties&, const SignalProperties& )
{
  samples=Parameter( "SampleBlockSize" );
  visualize= ( int )Parameter( "VisualizeClassFiltering" );
  n_vmat= Parameter( "MUD" )->NumRows();
  ...
  {
    delete vis;
    vis= new GenericVisualization( SourceID::Classifier );
    vis->Send(CfgID::WindowTitle, "Classifier");
...
void ClassFilter::Process(const GenericSignal& input, GenericSignal& output)
{
  ...
  {
    term1= input( vc1[i]-1, vf1[i]-1 );

    if( ( vc2[i] > 0  ) && ( vf2[i] > 0 ) )
    term2= input( vc2[i]-1, vf2[i]-1 );
    else    term2= 1.0;
  ...
  {
    if( ( hc1[i]>0 ) && (hf1[i] > 0 ) )
    term1= input( hc1[i]-1, hf1[i]-1 );
    else term1= 0;

    if( ( hc2[i] > 0  ) && ( hf2[i] > 0 ) )
    term2= input( hc2[i]-1, hf2[i]-1 );
    else    term2= 1.0;
